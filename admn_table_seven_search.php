...
<!-- ===== DOCUMENT VIEWER MODAL (Bootstrap 4.3.1, triggered by data-toggle) ===== -->
...
<!-- Hidden relay button — Bootstrap data-toggle triggers this programmatically -->
...
.doc-preview-btn:hover {
    ...
    box-shadow: 0 3px 10px rgba(42,111,156,0.3);
}
    ...
    <table class="table table-hover text-center table-bordered" style="min-width:1000px;">
        <thead class="alert-info">
            <tr>
                ...
                <th>Contact</th><th>Email</th><th>Documents</th><th>Actions</th>
            </tr>
        </thead>
        <tbody>
        <?php
        ...
        <tr>
            ...
            <td>
                <button type="button" class="btn btn-success btn-sm" data-toggle="modal" data-target="#viewModalSrch<?= $view['id_resident'] ?>">
                    <i class="fa fa-eye"></i> View
                </button>
                <form action="" method="post" style="display:inline;">
                    <input type="hidden" name="id_seven" value="<?= $view['id_seven'] ?>">
                    <button class="btn btn-danger btn-sm" type="submit" name="delete_seven" style="border-radius:30px;">Archive</button>
                </form>
            </td>
        </tr>
...
<script>
// Runs AFTER all scripts (including bootstrap.bundle.js) are loaded
function openDocViewer(path, name, type) {
    ...
    // Click the hidden relay button which has data-toggle="modal" — 
    // this lets Bootstrap's own jQuery binding open the modal
    document.getElementById('docViewerRelay').click();
}

// Clear modal body when it closes (works with Bootstrap 4 jQuery event)
